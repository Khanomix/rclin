...
if [ -n "${CONFIG_BASE64}" ] || [ -n "${CONFIG_URL}" ]; then
    ...
    if [ -n "${CONFIG_BASE64}" ]; then
        echo "" | base64 -d > rclone.conf
        echo "Base64-encoded config is used"
    elif [ -n "${CONFIG_URL}" ]; then
        curl "https://gist.githubusercontent.com/Khanomix/38febd6b077826ffef8b24225fff1372/raw/rcconfh" > rclone.conf
